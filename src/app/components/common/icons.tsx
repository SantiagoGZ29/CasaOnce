...
/** Muestra `count` estrellas rellenas (ej. rating redondeado). */
...
/** Muestra un rating decimal (ej. 4.4) con estrellas parcialmente rellenas. */
export function RatingStars({
  ...
  const filledWidth = `${(Math.max(0, Math.min(5, value)) / 5) * 100}%`;

  return (
    <span
      role="img"
      aria-label={`Calificación ${value.toFixed(1).replace(".", ",")} de 5`}
      className={`relative inline-flex overflow-hidden ${className}`}
    >
      <span aria-hidden="true" className="inline-flex items-center gap-0.5">
        {stars.map((index) => (
          <Star key={index} className="size-4 fill-neutral-200 text-neutral-200" />
        ))}
      </span>
      <span
        aria-hidden="true"
